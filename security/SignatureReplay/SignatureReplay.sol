...
/*
	签名重放
	在区块链中, 数字签名可以用于识别数据签名者和验证数据完整性; 发送交易时, 
	用户使用私钥签名交易, 使得其他人可以验证交易是由相应账户发出的; 智能合
	约也能利用 ECDSA 算法验证用户将在链下创建的签名, 然后执行铸造或转账等逻辑;

	数字签名一般有两种常见的重放攻击:
	1. 普通重放: 将本该使用一次的签名多次使用; NBA官方发布的《The Association》
		系列 NFT 因为这类攻击被免费铸造了上万枚
	2. 跨链重放: 将本该在一条链上使用的签名, 在另一条链上重复使用; 做市商 
		Wintermute 因为跨链重放攻击被盗2000万枚 $OP(TODO: 了解)

	预防:
	1. 将使用过的签名记录下来, 比如记录下已经铸造代币的地址 mintedAddress,
		防止签名反复使用:
	...
	2. 将 nonce (数值随每次交易递增)和 chainid (链ID)包含在签名消息中, 
		这样可以防止普通重放和跨链重放攻击：
...
contract SigReplay is ERC20 {
    ...
    function verify(bytes32 _msgHash, bytes memory _signature) public view returns (bool){
        return ECDSA.recover(_msgHash, _signature) == signer; // TODO
    }
}
