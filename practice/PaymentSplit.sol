...
/*
	分账
	分账就是按照一定比例分钱财, 在现实中经常会有"分赃不均"的事情发生; 而在区块
	链的世界里Code is Law, 可以事先把每个人应分的比例写在智能合约中, 获得收入
	后再由智能合约来进行分账	
	...
	4. 分账合约遵循Pull Payment模式, 付款不会自动转入账户, 而是保存在此合约中;
		受益人通过调用release()函数触发实际转账 (TODO: Pull Payment模式)
*/


/**
 * 分账合约 
 * @dev 这个合约会把收到的ETH按事先定好的份额分给几个账户, 
...
contract PaymentSplit {
     ...
     * 数组长度不能为0, 两个数组长度要相等, _shares中元素要大于0, _payees中
	 * 地址不能为0地址且不能有重复地址
     */
    ...
    /**
     * @dev 根据受益人地址`_account`, 分账合约总收入`_totalReceived`和该
	 * 地址已领取的钱`_alreadyReleased`, 计算该受益人现在应分的`ETH`
     */
    ...
    /**
     * @dev 新增受益人_account以及对应的份额_accountShares
	 * 只能在构造器中被调用, 不能修改
     */
